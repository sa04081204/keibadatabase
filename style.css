(() => {
  ...
  const DATA_VERSION = "v10"; // データ更新のたびに数字を上げるとキャッシュを確実に回避できる
  ...
  const state = {
    ...
    predictIndex: null,  // data/predictions/index.json
    predictCache: {},    // race_id -> 予想結果json
  };
  ...
  function bindTabs() {
    els(".tabbar button").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        ["overview", "ranking", "search", "predict"].forEach((t) => {
          el(`#tab-${t}`).hidden = t !== tab;
        });
        if (tab === "predict") loadPredictIndex();
      });
    });
  }

  // ---------------- AI予想 ----------------
  async function loadPredictIndex() {
    if (state.predictIndex) return;
    const sel = el("#pr-race");
    try {
      const res = await fetch(`data/predictions/index.json?${DATA_VERSION}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      state.predictIndex = data.races || [];
      if (!state.predictIndex.length) {
        el("#pr-result").innerHTML = `<div class="empty-note">まだ予想データがありません。tools/predict_race.py を実行してください。</div>`;
        return;
      }
      sel.innerHTML = `<option value="">予想レースを選択してください</option>` +
        state.predictIndex.map((r) => `<option value="${r.race_id}">${r.start_time || ""} ${r.race_name || r.race_id}（${r.surface}${r.distance}m）</option>`).join("");
      sel.addEventListener("change", () => renderPredict(sel.value));
      // レースが1つしかなければ自動で開く
      if (state.predictIndex.length === 1) {
        sel.value = state.predictIndex[0].race_id;
        renderPredict(sel.value);
      }
    } catch (err) {
      el("#pr-result").innerHTML = `<div class="empty-note">予想データの読み込みに失敗しました（${err.message}）。data/predictions/index.jsonを確認してください。</div>`;
    }
  }

  function scoreBar(score) {
    const pct = Math.max(0, Math.min(100, score));
    const color = pct >= 70 ? "var(--amber)" : pct >= 45 ? "var(--turf)" : "var(--text-mute)";
    return `<div class="score-bar-track"><div class="score-bar-fill" style="width:${pct}%; background:${color};"></div></div>`;
  }

  async function renderPredict(raceId) {
    const wrap = el("#pr-result");
    if (!raceId) { wrap.innerHTML = ""; el("#pr-disclaimer").textContent = ""; return; }
    wrap.innerHTML = `<div class="loading-note">読み込み中…</div>`;
    try {
      let data = state.predictCache[raceId];
      if (!data) {
        const res = await fetch(`data/predictions/${raceId}.json?${DATA_VERSION}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        data = await res.json();
        state.predictCache[raceId] = data;
      }
      el("#pr-disclaimer").textContent = `※${data.disclaimer || ""}`;
      wrap.innerHTML = `
        <div class="search-summary">${data.race_name || ""}／${data.surface}${data.distance}m／${data.field_size}頭立て／コース照合:${data.course_key}</div>
        <div class="entry-list">
          ${data.results.map((r) => `
            <div class="entry-row predict-row">
              <div class="line1"><span>${r.rank}位予測　${r.umaban ?? "-"}番　${r.sex || ""}${r.age ?? ""}　${r.kinryo ?? "-"}kg</span><span>score ${r.score}</span></div>
              <div class="line2"><span class="horse">${r.horse_name}</span><span class="finish">${r.jockey || ""}</span></div>
              ${scoreBar(r.score)}
              ${r.reasoning && r.reasoning.length ? `<div class="tags">${r.reasoning.map((t) => `<span class="tag">${t}</span>`).join("")}</div>` : ""}
            </div>
          `).join("")}
        </div>
      `;
    } catch (err) {
      wrap.innerHTML = `<div class="empty-note">予想データの読み込みに失敗しました（${err.message}）。</div>`;
    }
  }
